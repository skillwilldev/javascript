...
const emmaBirthYear = 1995;
const michaelBirthYear = 1998;

const emmaAge = currentYear - emmaBirthYear;
const michaelAge = currentYear - michaelBirthYear;

const emmaIsOlder = emmaAge > michaelAge;
const ageDifference = Math.abs(emmaAge - michaelAge);

console.log(`Emma is ${emmaAge} and Michael is ${michaelAge}.`);
console.log(`Is Emma older? ${emmaIsOlder}. Difference: ${ageDifference} years.`);
...
if (temp1C > temp2C) {
    console.log(`Temp 1 (${temp1C}°C / ${temp1F}°F) is higher than Temp 2 (${temp2C}°C / ${temp2F}°F).`);
} else if (temp2C > temp1C) {
    console.log(`Temp 2 (${temp2C}°C / ${temp2F}°F) is higher than Temp 1 (${temp1C}°C / ${temp1F}°F).`);
} else {
    console.log("Both temperatures are equal.");
}
...
const f1 = 8, f2 = 9, f3 = 7; // Change values for other test data
const average = (f1 + f2 + f3) / 3;
const minRatingMet = f1 >= 4 && f2 >= 4 && f3 >= 4;

let recommendation;

if (average >= 7 && minRatingMet) {
...
console.log(`Average: ${average.toFixed(1)}. Status: ${recommendation}`);
...
totalFee += isWeekend ? 2 : 0; // Weekend surcharge

console.log(`Parking for ${hours} hours. Total fee: $${totalFee}`);
...
const hw = 85, midterm = 78, final = 92;

const finalGrade = (hw * 0.2) + (midterm * 0.3) + (final * 0.5);
let letterGrade;
...
const passed = finalGrade >= 60 && final >= 50;

console.log(`Final Grade: ${finalGrade}. Letter: ${letterGrade}. Passed: ${passed}`);
...
const discountRate = orderAmount > 100 ? 0.15 : (orderAmount >= 50 ? 0.10 : 0);
let currentPrice = orderAmount * (1 - discountRate);

// Apply premium discount (on the remaining amount)
const premiumDiscount = isPremium ? currentPrice * 0.05 : 0;
const finalPrice = currentPrice - premiumDiscount;
const totalSaved = orderAmount - finalPrice;

console.log(`Original: $${orderAmount}`);
console.log(`Discount: $${totalSaved.toFixed(2)}`);
