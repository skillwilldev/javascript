...
// ჩელენჯი #1 - კალორიების ტრეკერი
//აქ ვიყენებთ Arrow ფუნქციებს და Template Literals-ს.
...
const describeCalories = (name, bmr) => {
    return `${name}-ს მოსვენებულ მდგომარეობაში დაახლოებით ${bmr} კალორია ესაჭიროება დღეში.`;
};

const johnBMR = calcBMR(80, 180, 30, 'M');
const sarahBMR = calcBMR(60, 165, 25, 'F');

console.log(describeCalories("John", johnBMR));
console.log(describeCalories("Sarah", sarahBMR));
///////////////////////////////////////////////////////////////////////////


// ჩელენჯი #2 - სტუდენტის ნიშნების მენეჯერი
// ამ ამოცანაში ვსწავლობთ ფუნქციების ერთმანეთთან დაკავშირებას.
// 1. საშუალოს გამოთვლა ციკლის გარეშე
const calcAverage = (scores) => (scores[0] + scores[1] + scores[2] + scores[3] + scores[4]) / scores.length;

// 3. ასოითი ნიშნის განსაზღვრა
...
const avg1 = calcAverage(scores1);
const grade1 = getLetterGrade(avg1);
const status1 = isPassing(avg1) ? "აბარებს" : "ჭრის";

console.log(`სტუდენტის საშუალო: ${avg1} (${grade1}) - ${status1}`);
///////////////////////////////////////////////////////////////////////////////

// ჩელენჯი #3 - სავაჭრო სიის მენეჯერი
// აქ ვიყენებთ მასივის ძირითად მეთოდებს: push, pop, shift, unshift, includes, indexOf.

// 1. საწყისი მასივი
const shoppingList = ['bread', 'eggs', 'milk', 'butter', 'cheese'];

// 2-3. ელემენტების დამატება
shoppingList.push('tomatoes', 'pasta');
shoppingList.unshift('water');

// 4-5. ელემენტების ამოღება
const removedLast = shoppingList.pop();
const removedFirst = shoppingList.shift();

// 6-7. ძებნა
console.log(`არის 'milk' სიაში? ${shoppingList.includes('milk')}`);
console.log(`'butter'-ის ინდექსი: ${shoppingList.indexOf('butter')}`);

// 8-9. შეჯამება
console.log(`საბოლოო სია: ${shoppingList}`);
console.log(`რაოდენობა: ${shoppingList.length}`);
console.log(`ბოლოდან ამოღებული: ${removedLast}, დასაწყისიდან ამოღებული: ${removedFirst}`);

////////////////////////////////////////////////////////////////////////
// ჩელენჯი #4 - გუნდის ქულების ტრეკერი
// აქ ყველაფერს ერთად ვაერთიანებთ.

const eaglesScores = [88, 95, 72];
const hawksScores = [90, 78, 85];

const calcTeamAverage = (arr) => (arr[0] + arr[1] + arr[2]) / 3;

const getTeamStatus = (avg) => {
...
const determineWinner = (name1, avg1, name2, avg2) => {
    if (avg1 > avg2) return `${name1} იმარჯვებს ${avg1.toFixed(1)} vs ${avg2.toFixed(1)}!`;
    if (avg2 > avg1) return `${name2} იმარჯვებს ${avg2.toFixed(1)} vs ${avg1.toFixed(1)}!`;
    return "ფრეა!";
};

const avgEagles = calcTeamAverage(eaglesScores);
const avgHawks = calcTeamAverage(hawksScores);
// 5. მასივის გაერთიანება ციკლის გარეშე
const allScores = [eaglesScores[0], eaglesScores[1], eaglesScores[2]];
allScores.push(hawksScores[0], hawksScores[1], hawksScores[2]);
// 6. მაქსიმუმის პოვნა ხელით
const maxScore = Math.max(...allScores); // ან მრავალი if-ით
console.log(determineWinner("Eagles", avgEagles, "Hawks", avgHawks));
console.log(`ყველა დროის მაქსიმალური ქულა: ${maxScore}`);

////////////////////////////////////////////////////////////////////////
// დამატებითი ჩელენჯები (ობიექტები და ციკლები)
// ამ ნაწილში ობიექტების this კონტექსტსა და for/while ციკლებს ვნახავთ.
// #5 კონტაქტების წიგნი
const contact1 = {
    firstName: 'Alice',
    lastName: 'Johnson',
    ...
    getSummary: function () {
        return `${this.getFullName()} (${this.age}) - ${this.email} - ${this.friends.length} მეგობარი`;
    }
};

contact1.phone = "555-1234"; // Dot notation
...
// While ციკლის მაგალითი (ტაიმერი)
let timer = 3;
while (timer > 0) {
    console.log(`დასვენება: ${timer} წამი დარჩენილია...`);
    timer--;
}
